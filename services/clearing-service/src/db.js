...
const db = new Database(path.join(dataDir, 'clearing.db'));
...
function recordCommit(sagaId, destino, importe) {
  db.prepare(`
    INSERT OR REPLACE INTO saga_commits (saga_id, destino, importe, compensated)
    VALUES (?, ?, ?, 0)
  `).run(sagaId, destino, importe);
}
